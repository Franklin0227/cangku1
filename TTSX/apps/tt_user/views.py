from django.shortcuts import render,redirect #redirect模块含义？？
...
# def register(request): #register=登录
...
#         '处理注册'
#         pass
# def registerHandle(request): #registerHandle=登录操作
#     '''
#     处理注册
#     '''
#     pass
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # 判断用户名是否存在
        if User.objects.filter(username=uname).count() > 0: #username??????
